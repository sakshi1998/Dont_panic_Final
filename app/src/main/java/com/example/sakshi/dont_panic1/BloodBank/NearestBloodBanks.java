...
import com.example.sakshi.dont_panic1.Hospital.Hospital_Compare;
...
public class NearestBloodBanks extends AppCompatActivity {
    ...
    public static boolean bloodLoaded=false;
    ...
    protected void fillList(Activity activity) {
      ...
      //  MainActivity.progressDialog.cancel();
        ...
        for(int i = 0; i< GeometryBlood.detailArrayList.size(); i++) {
            String s = GeometryBlood.detailArrayList.get(i).getHospitalName();
            check(s.toLowerCase(),i);
        }
        Collections.sort(GeometryBlood.detailArrayList,new BloodBank_Compare());

        Blood_udapter customPlacesAdapter = new Blood_udapter(activity,GeometryBlood.detailArrayList );
        MainActivity.centersListView.setAdapter(customPlacesAdapter);
        try {
            String s = GeometryBlood.detailArrayList.get(0).getHospitalName();
            double x = GeometryBlood.detailArrayList.get(0).getGeometry()[0];
            double y = GeometryBlood.detailArrayList.get(0).getGeometry()[1];
            showNotification(activity, s, x, y);
        }
         catch(IndexOutOfBoundsException e){
            Toast.makeText(activity,"Google API query limit exceded",Toast.LENGTH_LONG).show();
            loadLocation();
        }
    ...
    public void check(String s,int i){
        DatabaseReference rootRef = FirebaseDatabase.getInstance().getReference().child("bloodbank").child(s);
        rootRef.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                if (dataSnapshot.hasChild(s)) {
                    // run some code
                    String nob = dataSnapshot.child("bloodtype").child("hg").getValue().toString();
                    GeometryBlood.detailArrayList.get(i).setAvailability(nob);
                } else {
                    GeometryBlood.detailArrayList.get(i).setAvailability("yes");
                    rootRef.child(s).child("bloodtype").setValue("yes");
                }
            }
            @Override
