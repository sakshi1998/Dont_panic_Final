...
public class NearestPharmacy extends AppCompatActivity {
    ...
    public static boolean pharmacyLoaded=false;
    ...
    protected void fillList(Activity activity) {
       ...
       // MainActivity.progressDialog.cancel();
        ...
        for(int i = 0; i< GeometryPharmacy.detailArrayList.size(); i++) {
            String s = GeometryPharmacy.detailArrayList.get(i).getPharmacyName();
            check(s.toLowerCase(),i);
        }
        Collections.sort(GeometryPharmacy.detailArrayList,new Pharmacy_compare());

        //TO BE DONE

        pharmacy_udapter customPlacesAdapter = new pharmacy_udapter(activity,GeometryPharmacy.detailArrayList);
        MainActivity.centersListView.setAdapter(customPlacesAdapter);
        try {
            String s = GeometryPharmacy.detailArrayList.get(0).getPharmacyName();
            double x = GeometryPharmacy.detailArrayList.get(0).getGeometry()[0];
            double y = GeometryPharmacy.detailArrayList.get(0).getGeometry()[1];
    ...
    public void check(String s,int i){
        DatabaseReference rootRef = FirebaseDatabase.getInstance().getReference().child("pharmacy");
        rootRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                if (dataSnapshot.hasChild(s)) {
                    ...
                    GeometryPharmacy.detailArrayList.get(i).setAvailability(nob);
                } else {
                    GeometryPharmacy.detailArrayList.get(i).setAvailability(10);
